...
const aviso = `🫟 El bot *${setting.botname}* está desactivado en este grupo\n\n> ඩා Un *administrador* puede activarlo con el comando:\n> » *${usedPrefix}bot on*`
await m.reply(aviso)
return
}}
if (m.text && user.banned && !isMods) {
const mensaje = `🫟 Estas baneado/a, no puedes usar comandos en este bot!\n\n> ඩා *Razón ›* ${user.bannedReason}\n\n> ඞ Si este Bot es cuenta oficial y tienes evidencia que respalde que este mensaje es un error, puedes exponer tu caso con un mod del bot.`.trim()
if (!primaryBotId || primaryBotId === botId) {
m.reply(mensaje)
return
}}}
