...
if (!text) {
return m.reply('ඞ Por favor, ingresa el título o URL del video de *(xnxx)*.')
}
conn.xnxx = conn.xnxx || {}
...
await m.react('🕒')
if (!res.result?.length) return m.reply('⚠︎ No se encontraron resultados.')
await m.react('✔️')
...
await m.reply('⚠︎ Por favor, ingresa un número válido.')
return
}
try {
await m.react('🕒')
