...
try {
if (!db.data.chats[m.chat].nsfw && m.isGroup) return m.reply(`⚠︎ El contenido *NSFW* está desactivado en este grupo.\n\nUn *administrador* puede activarlo con:\n» *${usedPrefix}nsfw on*`)
if (!args[0]) return conn.reply(m.chat, `⚠︎ Ingresa un tag para buscar.`, m)
await m.react('🕒')
...
if (!mediaList.length) return conn.reply(m.chat, `⚠︎ No se encontraron resultados para *${tag}*`, m)
const media = mediaList[Math.floor(Math.random() * mediaList.length)]
const caption = `ඞ Resultados para » *${tag}*`
