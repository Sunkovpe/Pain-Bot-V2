...
switch (command) {
case 'banned': {
if (!who) return conn.reply(m.chat, 'ඞ Por favor, etiqueta, cita o escribe el número del usuario que quieres banear del Bot.', m)
...
if (who === conn.user.jid) return conn.reply(m.chat, `⚠︎ @${bot} No puede ser baneado.`, m, { mentions: [who] })
...
await m.react('✔️')
await conn.reply(m.chat, `ඞ ${nameBan} ha sido baneado.\n> Razón: ${reason}`, m, { mentions: [who] })
await conn.reply(`${suittag}@s.whatsapp.net`, `ඞ ${nameBan} fue baneado por ${await conn.getName(m.sender)}\n> ✦ Razón: ${reason}`, m)
break
}
case 'unban': {
if (!who) return conn.reply(m.chat, 'ඞ Por favor, etiqueta o coloca el número del usuario que quieres desbanear del Bot.', m)
if (!users[who]) return m.reply('ඞ El usuario no está registrado.', m)
if (!users[who].banned) return m.reply(`⚠︎ @${who.split('@')[0]} no está baneado.`, m, { mentions: [who] })
await m.react('🕒')
users[who].banned = false
users[who].bannedReason = ''
await m.react('✔️')
let nameUnban = await conn.getName(who)
await conn.reply(m.chat, `ඞ ${nameUnban} ha sido desbaneado.`, m, { mentions: [who] })
await conn.reply(`${suittag}@s.whatsapp.net`, `ඞ ${nameUnban} fue desbaneado por ${await conn.getName(m.sender)}.`, m)
break
}
case 'block': {
if (!who) return conn.reply(m.chat, 'ඞ Por favor, menciona al usuario que quieres bloquear del número de la Bot.', m)
await m.react('🕒')
await conn.updateBlockStatus(who, 'block')
await m.react('✔️')
conn.reply(m.chat, `ඞ Bloqueado correctamente a @${who.split('@')[0]}`, m, { mentions: [who] })
break
}
case 'unblock': {
if (!who) return conn.reply(m.chat, 'ඞ Por favor, menciona al usuario que quieres desbloquear del número de la Bot.', m)
await m.react('🕒')
await conn.updateBlockStatus(who, 'unblock')
await m.react('✔️')
conn.reply(m.chat, `ඞ Desbloqueado correctamente a @${who.split('@')[0]}`, m, { mentions: [who] })
break
}
case 'banlist': {
await m.react('🕒')
