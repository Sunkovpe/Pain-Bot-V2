...
case 'letra':
if (!text) {
return conn.reply(m.chat, 'ඞ Por favor, ingresa el texto que quieres transformar.', m)
}
...
case 'style':
if (!text) {
return conn.reply(m.chat, 'ඞ Por favor, ingresa la nueva fuente que quieres establecer <1|2|3>.', m)
}
let num = parseInt(text)
if (!yStr[num]) {
return conn.reply(m.chat, `⚠︎ Estilo ${num} no disponible. Usa del 1 al ${Object.keys(yStr).length}.`, m)
}
global.currentFontStyle = num
let name = styleNames[num] || `${num}`
let example = await global.style('Hola mundo', num)
await m.react('✔️')
conn.reply(m.chat, `ඩා Estilo de fuente cambiado a *(${name})*.\n> Ejemplo: *${example}*`, m)
break
}}
