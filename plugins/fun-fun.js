...
if (!global.db.data.chats[m.chat].gacha && m.isGroup) return m.reply(`ඞ Los comandos de *Gacha* están desactivados en este grupo.\n\nUn *administrador* puede activarlos con el comando:\n» *${usedPrefix}gacha on*`)
try {
...
if (!texto) return conn.reply(m.chat, `ඞ Por favor, ingrese un texto luego del número para hacer un Top *cantidad texto*`, m)
if (ps.length < cantidad) return conn.reply(m.chat, `⚠︎ No hay suficientes miembros para hacer un Top ${cantidad}`, m)
let seleccionados = []
...
if (command == 'sorteo') {
if (!text) return conn.reply(m.chat, `ඞ Por favor ingresa lo que deseas sortear.`, m)
let cantidad = 1
let premio = text
if (!isNaN(parseInt(args[0]))) {
cantidad = Math.min(Math.max(parseInt(args[0]), 1), 10)
...
if (!premio) return conn.reply(m.chat, `ඞ Por favor, ingrese un texto luego del número para hacer el sorteo.`, m)
if (ps.length < cantidad) return conn.reply(m.chat, `⚠︎ No hay suficientes miembros para seleccionar ${cantidad} ganador(es).`, m)
let seleccionados = []
while (seleccionados.length < cantidad) {
let candidato = ps[Math.floor(Math.random() * ps.length)]
if (!seleccionados.includes(candidato)) seleccionados.push(candidato)
}
let mensaje = cantidad === 1 ? `ᥬ 𝑭𝑬𝑳𝑰𝑪𝑰𝑫𝑨𝑫𝑬𝑺 ​᭄\n\nඞ ${user(seleccionados[0])}\n○ Haz ganado un *${premio}*` : `ᥬ 𝑭𝑬𝑳𝑰𝑪𝑰𝑫𝑨𝑫𝑬𝑺 ​᭄\n\n` + seleccionados.map((u, i) => `${i + 1}. ${user(u)}`).join('\n') + `\n\n○ Han ganado un *${premio}*`
...
if (!text) return conn.reply(m.chat, `ඞ Escribe tu nombre y el nombre de la otra personas para calcular su amor.`, m)
...
user.afk = Date.now()
user.afkReason = text
return await conn.reply(m.chat, `ඞ *El Usuario ${await conn.getName(m.sender)} Estará AFK*\n○ *Motivo${text ? ': ' + text : ': Sin Especificar!'}*`, m)
}
...
if (!userId) return conn.reply(m.chat, `ඞ Por favor, ingrese el nombre de alguna persona.`, m)
...
let ps = groupMetadata.participants.map(v => v.id)
if (ps.length < cantidad * 2) return m.reply(`⚠︎ No hay miembros suficientes para formar ${cantidad} pareja${cantidad === 1 ? '' : 's'}`)
let usados = new Set()
let parejas = []
let menciones = []
...
if (!usser) return conn.reply(m.chat, `ඞ Por favor, mensiona a un Usuario para comprobar su test.`, m)
...
if (!userId) return conn.reply(m.chat, `ඞ Por favor, ingrese el tag de algún usuario o responda a un mensaje.`, m)
