...
if (!text) return conn.sendMessage(m.chat, { text: 'ඞ Por favor, responde a un mensaje o ingresa un texto para crear el Sticker.' }, { quoted: m })
await m.react('🕒')
const buffer = await fetchSticker(text)
const stiker = await sticker(buffer, false, texto1, texto2)
if (!stiker) throw new Error('⚠︎ No se pudo generar el sticker.')
...
case 'emojimix': {
if (!args[0]) return m.reply(`ඞ Ingresa 2 emojis para combinar.\n> Ejemplo: *${usedPrefix + command}* 👻+👀`)
...
if (!res.results || res.results.length === 0) throw new Error('⚠︎ No se encontraron stickers para esos emojis.')
...
if (!textFinal) return conn.reply(m.chat, `ඞ Ingresa un texto para crear el sticker.`, m)
...
if (frase.length > 30) return await m.react('✖️'), conn.reply(m.chat, `⚠︎ El texto no puede tener más de 30 caracteres.`, m)
await m.react('🕒')
...
case 'take': case 'wm': {
if (!m.quoted) return m.reply(`ඞ Responde a un sticker con el comando *${usedPrefix + command}* seguido del nuevo nombre.\n> Ejemplo: *${usedPrefix + command}* NuevoNombre`)
await m.react('🕒')
const stickerData = await m.quoted.download()
if (!stickerData) return await m.react('✖️'), m.reply('⚠︎ No se pudo descargar el sticker.')
