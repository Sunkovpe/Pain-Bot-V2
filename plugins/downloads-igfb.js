...
try {
if (!args[0]) return conn.reply(m.chat, `ඞ Por favor, ingresa un enlace de Instagram/Facebook.`, m)
let data = []
try {
await m.react('🕒')
...
if (!data.length) return conn.reply(m.chat, `⚠︎ No se pudo obtener el contenido.`, m)
for (let media of data) {
await conn.sendFile(m.chat, media, 'instagram.mp4', `ඞ Aquí tienes`, m)
await m.react('✔️')
}} catch (error) {
await m.react('✖️')
