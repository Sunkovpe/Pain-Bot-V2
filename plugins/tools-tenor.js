...
if (!text) return m.reply(`ඞ Por favor, ingrese el texto para buscar su Gifs.`)
try {
await m.react('🕒')
const res = await fetch(`${global.APIs.delirius.url}/search/tenor?q=${text}`)
const json = await res.json()
const gifs = json.data
if (!gifs || gifs.length < 2) { await m.react('✖️'); return m.reply('⚠︎ No se encontraron resultados.') }
const maxItems = Math.min(gifs.length, 10)
const medias = gifs.slice(0, maxItems).map(gif => ({ type: 'video', data: { url: gif.mp4 } }))
await sendAlbumMessage(m.chat, medias, { caption: `ඩා G I F - S E A R C H ඩා\n\n✦ Busqueda: ${text}\n✧ Resultados: ${maxItems}`, quoted: m })
await m.react('✔️')
} catch (e) {
await m.react('✖️')
