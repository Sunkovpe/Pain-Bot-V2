...
switch (command) {
case 'dalle': {
if (!args[0]) return conn.reply(m.chat, `ඞ Por favor, proporciona una descripción para generar la imagen.`, m)
const promptDalle = args.join(' ')
if (promptDalle.length < 5) return conn.reply(m.chat, `⚠︎ La descripción es demasiado corta.`, m)
await m.react('🕒')
...
case 'flux': {
if (!text) return conn.reply(m.chat, `ඞ Por favor, ingrese un término para generar la imagen`, m)
await m.react('🕒')
const result = await fluximg.create(text)
if (result?.imageLink) {
await conn.sendMessage(m.chat, { image: { url: result.imageLink }, caption: `ඩා *Resultados de:* ${text}` }, { quoted: m })
await m.react('✔️')
} else throw new Error("No se pudo crear la imagen")
break
}
case 'ia': case 'chatgpt': {
if (!text) return conn.reply(m.chat, `ඞ Ingrese una petición.`, m)
await m.react('🕒')
