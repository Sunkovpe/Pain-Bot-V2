...
switch (command) {
case 'saveplugin': {
if (!text) return m.reply(`ඞ Por favor, ingrese el nombre del plugin.`)
...
await m.react('✔️')
m.reply(`ඩා Guardando plugin en ${ruta}`)
break
}
case 'savefile': {
if (!text) return m.reply(`ඞ Ingresa la Ruta y el nombre del Archivo junto al comando.`)
if (!m.quoted?.text) return m.reply(`⚠︎ Responde al mensaje.`)
await m.react('🕒')
const path = `${text}.js`
await fs.writeFileSync(path, m.quoted.text)
await m.react('✔️')
m.reply(`ඩා Guardado en *${path}*.`)
break
}
case 'deletefile': {
if (!text) return conn.reply(m.chat, `ඞ Ingresa la ruta y el nombre del archivo que deseas eliminar.`, m)
const file = text.trim()
if (!existsSync(file)) return conn.reply(m.chat, `⚠︎ Archivo no encontrado.`, m)
await m.react('🕒')
unlinkSync(file)
await m.react('✔️')
conn.reply(m.chat, `ඞ El archivo *${file}* ha sido eliminado con éxito.`, m)
break
}
case 'getplugin': {
if (!text) return conn.reply(m.chat,`ඞ Ingrese el nombre de algún plugin existente*\n\n*—◉ Ejemplo*\n*◉ ${usedPrefix + command}* info-infobot\n\n*—◉ Lista de plugins:*\n*◉* ${ar1.map(v => ' ' + v).join`\n*◉*`}`, m)
