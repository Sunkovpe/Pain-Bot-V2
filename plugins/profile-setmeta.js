...
return m.reply(`ඞ Por favor, escribe el pack y el autor que deseas usar por defecto para tus stickers.\n> Ejemplo: *${usedPrefix + command} Ola • Ola*`)
}
...
return m.reply(`ඩා Se actualizo el pack y autor por defecto para tus stickers.`);
}
if (command === 'delmeta') {
if (!global.db.data.users[userId] || (!global.db.data.users[userId].text1 && !global.db.data.users[userId].text2)) {
return m.reply(`⚠︎ No tienes establecido un pack de stickers.`)
}
const packstickers = global.db.data.users[userId]
delete packstickers.text1
delete packstickers.text2
await global.db.write()
return m.reply(`ඩා Se restablecio el pack y autor por defecto para tus stickers.`)
}}
