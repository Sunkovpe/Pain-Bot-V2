...
switch (command) {
case 'setbirth': {
if (!text) return conn.reply(m.chat, `ඩා Debes ingresar una fecha válida para tu cumpleaños.\n\n> ✐ Ejemplo » *${usedPrefix + command} 01/01/2000* (día/mes/año)`, m)
...
if (!birth) {
return conn.reply(m.chat, `⚠︎ La fecha ingresada no es válida o no tiene lógica.\n> Ejemplo: *${usedPrefix + command} 01/12/2000*`, m)
}
user.birth = birth
return conn.reply(m.chat, `ඩා Se ha establecido tu fecha de nacimiento como: *${user.birth}*!`, m)
break
}
case 'delbirth': {
if (!user.birth) {
return conn.reply(m.chat, `⚠︎ No tienes una fecha de nacimiento establecida que se pueda eliminar.`, m)
}
user.birth = ''
return conn.reply(m.chat, `ඩා Tu fecha de nacimiento ha sido eliminada.`, m)
break
}
case 'setgenre': {
if (!text) return conn.reply(m.chat, `ඞ Debes ingresar un género válido.\n> Ejemplo » *${usedPrefix + command} hombre*`, m)
...
if (!genre) {
return conn.reply(m.chat, `⚠︎ Recuerda elegir un género válido.\n> Ejemplo: ${usedPrefix + command} hombre`, m)
}
if (user.genre === genre) {
return conn.reply(m.chat, `⚠︎ Ya tienes establecido el género como *${user.genre}*.`, m)
}
user.genre = genre
return conn.reply(m.chat, `ඩා Se ha establecido tu género como: *${user.genre}*!`, m)
break
}
case 'delgenre': {
if (!user.genre) {
return conn.reply(m.chat, `⚠︎ No tienes un género asignado.`, m)
}
user.genre = ''
return conn.reply(m.chat, `ඞ Se ha eliminado tu género.`, m)
break
}
case 'setdescription': case 'setdesc': {
if (!text) return conn.reply(m.chat, `⚠︎ Debes especificar una descripción válida para tu perfil.\n\n> ✐ Ejemplo » *${usedPrefix + command} Hola, uso WhatsApp!*`, m)
user.description = text
return conn.reply(m.chat, `ඩා Se ha establecido tu descripcion, puedes revisarla con #profile`, m)
break
}
case 'deldescription': case 'deldesc': {
if (!user.description) {
return conn.reply(m.chat, `⚠︎ No tienes una descripción establecida que se pueda eliminar.`, m)
}
user.description = ''
return conn.reply(m.chat, `ඩා Tu descripción ha sido eliminada.`, m)
break
}}} catch (error) {
