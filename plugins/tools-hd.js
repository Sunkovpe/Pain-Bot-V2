...
if (!mime) return conn.reply(m.chat, 'ඞ Por favor, responde a una imagen con el comando.', m)
if (!/image\/(jpe?g|png)/.test(mime)) return conn.reply(m.chat, `⚠︎ Formato no compatible (${mime}). Usa una imagen jpg o png.`, m)
...
await conn.sendFile(m.chat, Buffer.isBuffer(result) ? result : result, 'imagen.jpg', `ඩා Imagen mejorada`, m)
await m.react('✔️')
} catch (err) {
await m.react('✖️')
