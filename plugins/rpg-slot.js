...
return m.reply(`ඞ Por favor, ingresa la cantidad que deseas apostar.`)
}
const apuesta = parseInt(args[0])
if (Date.now() - users.lastslot < 10000) {
const restante = users.lastslot + 10000 - Date.now()
return m.reply(`⚠︎ Debes esperar *${formatTime(restante)}* para usar *${usedPrefix + command}* nuevamente.`)
}
if (apuesta < 100) return m.reply(`⚠︎ El mínimo para apostar es de 100 *${currency}*.`)
if (users.coin < apuesta) return m.reply(`⚠︎ Tus *${currency}* no son suficientes para apostar esa cantidad.`)
const emojis = ['✾', '❃', '❁']
...
let resultado
if (x[0] === y[0] && y[0] === z[0]) {
resultado = `ඩා Ganaste! *¥${(apuesta * 2).toLocaleString()} ${currency}*.`
users.coin += apuesta
} else if (x[0] === y[0] || x[0] === z[0] || y[0] === z[0]) {
resultado = `ඩා Casi lo logras. *Toma ¥10 ${currency}* por intentarlo.`
users.coin += 10
} else {
resultado = `ඩා Perdiste *¥${apuesta.toLocaleString()} ${currency}*.`
users.coin -= apuesta
}
users.lastslot = Date.now()
const finalText = `ඩා *SLOTS* 
────────
${x[0]} : ${y[0]} : ${z[0]}
${x[1]} : ${y[1]} : ${z[1]}
${x[2]} : ${y[2]} : ${z[2]}
