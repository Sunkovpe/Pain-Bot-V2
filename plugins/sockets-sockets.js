...
if (!isSubBots) return m.reply(`ඞ El comando *${command}* solo puede ser ejecutado por el Socket.`)
switch (command) {
...
if (enable || disable) {
if (isEnable === enable)
return m.reply(`⚠︎ El modo *${type}* ya estaba ${enable ? 'activado' : 'desactivado'}.`)
config[type] = enable
return conn.reply(m.chat, `ඩා Has *${enable ? 'activado' : 'desactivado'}* el modo *${type}* para el Socket.`, m)
}
conn.reply(m.chat, `ඩා Puedes activar o desactivar el modo *${type}* utilizando:\n\n● Activar » ${usedPrefix}${command} enable\n● Desactivar » ${usedPrefix}${command} disable\n\n✧ Estado actual » *${isEnable ? '✓ Activado' : '✗ Desactivado'}*`, m)
break
}
case 'join': {
if (!text) return m.reply(`ඞ Debes enviar un enlace de invitación para unirme a un grupo.`)
const [_, code] = text.match(linkRegex) || []
if (!code) return m.reply(`⚠︎ El enlace de invitación no es válido.`)
await m.react('🕒')
await conn.groupAcceptInvite(code)
await m.react('✔️')
m.reply(`ඩා ${botname} se a unido exitosamente al grupo.`)
break
}
...
chat.welcome = false
await conn.reply(id, `ඩා Adiós a todos, ${botname} se despide! (≧ω≦)ゞ`)
...
return conn.reply(m.chat, 'ඞ Este comando está deshabilitado en las sesiones principales.', m)
...
const cleanId = jidDecode(rawId)?.user || rawId.split('@')[0]
const sessionPath = path.join(global.jadi, cleanId)
if (!fs.existsSync(sessionPath)) return conn.reply(m.chat, 'ඞ Este comando solo puede ejecutarse desde una instancia Sub-Bot.', m)
await m.react('🕒')
if (typeof global.reloadHandler !== 'function')
throw new Error('No se encontró la función global.reloadHandler')
await global.reloadHandler(true)
await m.react('✔️')
conn.reply(m.chat, 'ඩා La sesión fue recargada correctamente.', m)
break
}}} catch (error) {
await m.react('✖️')
