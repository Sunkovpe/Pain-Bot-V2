...
try {
const pp = await conn.profilePictureUrl(m.chat, 'image').catch(() => 'https://files.catbox.moe/xd4vrj.jpeg')
...
return m.reply(`ඞ Debés mencionar o citar un mensaje de un usuario para aplicar una advertencia.\n> Ejemplo: *${usedPrefix + command} @usuario (motivo | opcional)*`)
}
...
if (who === conn.user.jid) return conn.reply(m.chat, `⚠︎ No puedo ponerle advertencias al bot.`, m)
if (who === ownerGroup) return conn.reply(m.chat, `⚠︎ No puedo darle advertencias al propietario del grupo.`, m)
if (who === ownerBot) return conn.reply(m.chat, `⚠︎ No puedo darle advertencias al propietario del bot.`, m)
user.warn = (user.warn || 0) + 1
await m.reply(`*@${who.split`@`[0]}* recibió una advertencia en este grupo!\nMotivo: ${motivo}\n*Advertencias: ${user.warn}/3*`, null, { mentions: [who] })
if (user.warn >= 3) {
user.warn = 0
await m.reply(`ඩා ¡Te lo advertí varias veces!\n*@${who.split`@`[0]}* superó las *3* advertencias, ahora será eliminado/a.`, null, { mentions: [who] })
await conn.groupParticipantsUpdate(m.chat, [who], 'remove')
}
break
}
case 'delwarn': case 'unwarn': {
if (!who) return m.reply(`ඞ Etiqueta a un usuario para quitarle las advertencias.`)
if (mentionedJid.includes(conn.user.jid)) return
if (user.warn === 0) throw `⚠︎ El usuario tiene 0 advertencias.`
user.warn -= 1
