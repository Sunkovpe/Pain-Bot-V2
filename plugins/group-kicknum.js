...
try {
if (!args[0]) return conn.reply(m.chat, `ඞ Ingrese algún prefijo de un país para ejecutar el comando.`, m)
if (isNaN(args[0])) return conn.reply(m.chat, `⚠︎ Ingrese algún prefijo de un país\nEjemplo: ${usedPrefix + command} 212`, m)
const lol = args[0].replace(/[+]/g, '')
const ps = participants.map(u => u.id).filter(v => v !== conn.user.jid && v.startsWith(lol))
const bot = global.db.data.settings[conn.user.jid] || {}
if (ps.length === 0) return m.reply(`⚠︎ Aquí no hay ningún número con el prefijo +${lol}`)
const numeros = ps.map(v => '⭔ @' + v.replace(/@.+/, ''))
const delay = time => new Promise(res => setTimeout(res, time))
switch (command) {
case 'listanum': case 'listnum': {
conn.reply(m.chat, `ඩා Lista de números con el prefijo +${lol} que están en este grupo:\n\n` + numeros.join`\n`, m, { mentions: ps })
break
}
case 'kicknum': {
