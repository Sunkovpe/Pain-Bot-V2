...
if (isNaN(args[0])) return conn.reply(m.chat, `⚠︎ Solo se aceptan números.\n> Ejemplo: *${usedPrefix + command} 1 h*`, m)
let kk = args[1]?.toLowerCase() || "h"
let precios = { h: pHora, d: pDia, s: pSemana, m: pMes }
let comisiones = { h: cHora, d: cDia, s: cSemana, m: cMes }
if (!precios[kk]) return conn.reply(m.chat, `⚠︎ Formato no válido. Opciones: h, d, s, m`, m)
let precio = precios[kk]
let comision = comisiones[kk]
let total = (precio * args[0]) + (comision * args[0])
if (user.coin < total) return conn.reply(m.chat, `⚠︎ No tienes suficientes ${currency} para comprar la membresía premium!`, m)
