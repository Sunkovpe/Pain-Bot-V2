...
return conn.reply(m.chat, `⚠︎ Debes esperar *${tiempoRestante}* para usar *${usedPrefix + command}* de nuevo.`, m)
}
user.lastwork = Date.now() + cooldown
let rsl = Math.floor(Math.random() * 1501) + 2000
await conn.reply(m.chat, `ඩා ${pickRandom(trabajo)} *¥${rsl.toLocaleString()} ${currency}*.`, m)
user.coin += rsl
}
