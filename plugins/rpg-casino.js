...
return conn.reply(m.chat, `⚠︎ Debes esperar *${tiempoRestante}* para usar *${usedPrefix + command}* nuevamente.`, m)
}
...
if (args.length < 1) {
return conn.reply(m.chat, `ඞ Ingresa la cantidad de *${currency}* que deseas aportar contra *${botname}*\n> Ejemplo: *${usedPrefix + command} 100*`, m)
}
if (user.coin >= count) {
user.coin -= count
let resultado = ''
...
const replyMsg = `ඩා \`Veamos qué números tienen!\`\n\n➠ *${botname}* : ${Aku}\n➠ *${userName}* : ${Kamu}\n\n` +
resultado
conn.reply(m.chat, replyMsg.trim(), m)
} else {
conn.reply(m.chat, `⚠︎ No tienes *¥${formatNumber(count)} ${currency}* para apostar!`, m)
}}
