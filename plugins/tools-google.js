...
if (!text) return m.reply(`ඞ Por favor, proporciona el término de búsqueda que deseas realizar a *Google*.\n\nEjemplo: ${usedPrefix}google gatos curiosos`)
...
await m.react('✖️')
return m.reply('⚠︎ No se encontraron resultados para esa búsqueda.')
}
let replyMessage = `✦ Resultados de la búsqueda para: *${text}*\n\n`
result.data.slice(0, maxResults).forEach((item, index) => {
replyMessage += `ඞ Título: *${index + 1}. ${item.title || 'Sin título'}*\n`
