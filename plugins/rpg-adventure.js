...
if (user.health < 5)
return conn.reply(m.chat, `⚠︎ No tienes suficiente salud para volver a *aventurarte*.\n> Usa *"${usedPrefix}heal"* para curarte.`, m)
const cooldown = 20 * 60 * 1000
const now = Date.now()
if (now < user.lastAdventure) {
const restante = user.lastAdventure - now
const wait = formatTime(restante)
return conn.reply(m.chat, `⚠︎ Debes esperar *${wait}* para usar *${usedPrefix + command}* de nuevo.`, m)
}
...
const resultado = `ඩා ${evento.mensaje} ${evento.tipo === 'neutro' ? '' : evento.tipo === 'victoria' ? `ganaste. *¥${monedas.toLocaleString()} ${currency}*` : `perdiste. *¥${monedas.toLocaleString()} ${currency}*`}`
