...
if (!text) {
return conn.reply(m.chat, `ඞ Te faltó el link de una imagen/video de twitter.`, m)
}
try {
await m.react('🕒')
const result = await twitterScraper(text);
if (!result.status) return conn.reply(m.chat, `⚠︎ No se pudo obtener el contenido de Twitter`, m)
